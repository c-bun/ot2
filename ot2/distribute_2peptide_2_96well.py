...
def replicate_plate(
    ...
    origin: protocol_api.labware.Labware,
    destinations: List[protocol_api.labware.Labware],
    amount: int,
):
    """
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for n, dest_plate in enumerate(well96_plates):
        ...
        amounts["peptides"][n % len(PEPTIDE_WELLS) + 1] += (
            PEPTIDE_AMOUNT * 8 * len(well96_plates[n].rows())
        )
